...
describe('RouteRegistry - Middleware Integration', () => {
  ...
  let registeredRoutes: Map<string, { path: string; handler: RouteHandler }>;
  ...
  describe('pipes', () => {
    ...
    it('should handle pipes returning Observables', async () => {
      ...
      await registeredRoutes.get('POST:/test')?.handler!(mockUwsRes, mockUwsReq);
  ...
  describe('full middleware pipeline', () => {
    it('should execute guards and handler in order', async () => {
      const executionOrder: string[] = [];

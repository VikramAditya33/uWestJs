...
import type { UwsResponse } from './uws-response';
...
export class CompressionHandler {
  ...
  constructor(options: CompressionOptions = {}) {
    ...
    this.options = {
      ...
      maxInflatedBodySize: options.maxInflatedBodySize ?? DEFAULT_OPTIONS.maxInflatedBodySize,
  ...
  private isCompressible(contentType: string | string[]): boolean {
    ...
    if (
      ...
      typeStr.includes('json') ||
      typeStr.includes('xml') ||
      typeStr.includes('javascript') ||
      typeStr.includes('ecmascript') ||
      typeStr.includes('svg')
    ) {
      return true;
    }
    ...
    if (
      typeStr.includes('gzip') ||
      typeStr.includes('zip') ||
      typeStr.includes('compress') ||
